...
python make_figures.py                             # 重画上面那张图
```
...
答案是:什么也没描述,而且不是差个常数。在摆上,它把典型误差**第 40 步高估 6780 倍**,
而且实测曲线是幂律、界是指数——**这条界连函数形式都是错的。**
原因是这个系统的 Lyapunov 指数只有 `+0.016`,基本为零,**没有什么可放大的**。
你看到的是误差在**累加**,不是在复合。
...
**每步都新注入一份误差,并不改变增长率。** 增长率由动力学决定,模型只决定起点多大。
注入做的事是把整条曲线抬高一个常数——本次实测按估计量不同在 13 到 30 之间,
跨种子在 8 到 28 之间。
...
| 第 1–2 课及其 25 条 README 断言 | ✓ | ✓ |
| 第 3–4 课及其 16 条断言 | ✗ 需要 GPU | ✓ |
| 第 5–6 课及其 19 条断言 | ✗ 需要 MuJoCo | ✓ |

CI 给 `check_claims.py` 传的是 `--lessons=1,2`,它会**打印出自己担保的那个子集**,
而不是给一个笼统的通过。其余断言在产出这些数字的机器上本地核验,60/60 那个徽章指的是完整运行。

`tests/` 不是常规单元测试。每一条都是**答案可以独立知道**的情形——
Lorenz 指数对文献值、有限差分 Jacobian 对解析解、已知形状的合成曲线、
置信区间对其名义覆盖率——**外加负对照**,后者抓到的更多:
一个在无结构数据上也能给出像样数字的指标,在坏模型上同样会给出像样的数字。

其中两条测试的存在是因为有 bug 逃过了别的所有检查:
`test_action_dim_is_not_hard_coded`(一个写错的常数,埋在两个函数里,
直到第 6 课用两关节机械臂才暴露),以及
`test_the_interval_covers_the_truth_about_95pc_of_the_time`
(自助法重采样的是单条平均曲线上的点而不是轨迹,把每个区间都缩窄了一半,
足以把"与 λ 一致"变成"与 λ 显著不同")。
